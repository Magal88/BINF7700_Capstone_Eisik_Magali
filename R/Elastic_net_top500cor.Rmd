...
```{r}
all_coef <- as.matrix(coef(enet_model))
coef_df_full <- data.frame(
  CpG = rownames(all_coef),
  Weight = as.numeric(all_coef)
)
top50_cpgs <- coef_df_full %>%
  filter(grepl("^cg", CpG)) %>%              
  arrange(desc(abs(Weight))) %>%            
  head(50) %>%
  mutate(Methylation_Change = ifelse(Weight > 0,
                                     "Hypermethylated",
                                     "Hypomethylated"))
write.csv(top50_cpgs %>% select(CpG, Weight, Methylation_Change),
          "Top50_CpGs.csv",
          row.names = FALSE)
```
